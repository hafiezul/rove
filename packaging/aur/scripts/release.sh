...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Rove Code-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit rove-icon.png
git config user.name 'rove-ci'
git config user.email 'rove-ci@users.noreply.github.com'
git add -A
