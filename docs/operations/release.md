...
The job looks up the `rove-marketing` project using the existing `VERCEL_TOKEN`
...
- Publishes the CLI npm packages (`t3` and `@rove/t3-<platform>-<arch>`) to the `nightly` npm dist-tag using the same nightly version.
...
into `~/.rove/wsl-runtime/sha256-<archive-digest>` inside the selected distro,
...
each `@rove/t3-<platform>-<arch>.tgz` and finally on `t3.tgz`, the launcher. The script publishes
...
packages are published per release: `t3`, `@rove/t3-darwin-arm64`, `@rove/t3-darwin-x64`,
`@rove/t3-linux-arm64`, `@rove/t3-linux-x64`, `@rove/t3-win32-arm64`,
`@rove/t3-win32-x64`.

Checklist:

1. Confirm the npm org owns package `t3` and the `@rove` scope exists on npm (create the org if
   it does not).
2. For `t3` and each `@rove/t3-<platform>-<arch>` package, configure a Trusted Publisher in the
