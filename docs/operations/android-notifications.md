...
On Android 16+, open Rove Code Settings → Live Update Settings to allow status bar chips. Android controls this separately from notification permission. The chip reads `Working` during work, `Approve` for approvals, and `Answer` for input requests; completed work returns to a normal notification. Use an Android 16 QPR2 or newer emulator image to verify the shipped promotion behavior.
...
1. Create a Firebase project and register each Android application identifier you intend to build: `dev.rove.app.dev`, `dev.rove.app.preview`, or `dev.rove.app`.
2. Download `google-services.json`. Set `ROVE_ANDROID_GOOGLE_SERVICES_FILE` to its path when running Expo prebuild and building the app. The JSON must contain the selected variant's package identifier.
...
ROVE_ANDROID_GOOGLE_SERVICES_FILE=/absolute/path/google-services.json \
vp run android:dev
```
...
```text
clerk://dev.rove.app.dev.callback
```
...
3. From the repository root, inspect the deployment plan, then deploy the same stage:
   ...
   ```sh
   vp run --filter rove-relay deploy --stage dev_ryan_android --env-file .env.android-dev --dry-run
   vp run --filter rove-relay deploy --stage dev_ryan_android --env-file .env.android-dev
   ```
