...
[rove-connect-auth-flow.html](./rove-connect-auth-flow.html).
...
```dotenv
ROVE_CLERK_PUBLISHABLE_KEY=<publishable key>
ROVE_CLERK_JWT_TEMPLATE=<JWT template name>
ROVE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
ROVE_RELAY_URL=https://relay.example.com
```
...
should set `ROVE_CLERK_PUBLISHABLE_KEY`, `ROVE_CLERK_JWT_TEMPLATE`,
`ROVE_CLERK_CLI_OAUTH_CLIENT_ID`, and `ROVE_RELAY_URL` before building. EAS preview and
...
deployment defaults.
`vp run --filter rove-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.t3.codes/connect/callback` for the hosted out-of-band flow. This is
     ...
     custom `ROVE_HOSTED_APP_URL` means `$ROVE_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `ROVE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
| ------- | ------------------------- |
| Name    | `t3-relay`                |
| Claims  | `{ "aud": "rove-relay" }` |

Set `ROVE_CLERK_JWT_TEMPLATE=t3-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=rove-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `ROVE_RELAY_URL` still
...
Local desktop development uses `rove-dev://app`, while packaged builds use `rove://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `rove-dev://app`; the production Clerk instance should only need
`rove://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `dev.rove.app`. To enable native passkeys:

1. Create an explicit macOS App ID for `dev.rove.app` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.dev.rove.app`.
...
ROVE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/rove.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
ROVE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `ROVE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`ROVE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `ROVE_PORT` set. Rebuild the signed app
