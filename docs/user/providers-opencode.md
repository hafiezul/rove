...
Rove Code requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let Rove Code start OpenCode locally. A password in
provider settings applies to both that server and Rove Code's connection. With no
...
settings. Rove Code uses only that configured password for an external server; it
...
restart before Rove Code can see configuration changes.
