...
Rove requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let Rove start OpenCode locally. A password in
provider settings applies to both that server and Rove's connection. With no
...
settings. Rove uses only that configured password for an external server; it
...
restart before Rove can see configuration changes.
