...
Rove Code uses Claude Code's login and configuration. Start with the default provider
...
Rove Code uses the Claude configuration on the connected server.
...
Select the model you want in Rove Code. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in Rove Code. Custom models may have
