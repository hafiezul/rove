...
This guide is for people who want to use more than one Claude setup in Rove. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Rove](./install.md).
...
In Rove Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Rove uses Claude Code's normal config directory.

When you set this field, Rove points Claude Code at that directory with the
...
`~/.claude_personal_home/.claude`, which is not where Rove looks.

Then add another Claude provider in Rove:
...
Rove only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Rove keeps separate config directories isolated
...
Add or edit a Claude provider in Rove Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Rove stores the value as a server secret and does not
...
Rove does not need a special Claude Code Router provider. Treat the router as a Claude
