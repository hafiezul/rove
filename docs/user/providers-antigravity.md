...
Rove runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Rove to confirm
...
hostname. Only that Rove sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Rove's separate Plan mode
...
Rove keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Rove also reads `.gemini/skills` and the
