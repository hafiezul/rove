...
Rove shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Rove directly from a
...
- **T3 Connect** connects computers that are signed in to your account. Run
  `npx t3 connect` on each computer you want to add, then start Rove or run
...
If Rove cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Rove cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Rove checks each selected computer for Claude Code and Codex. If an agent is
...
Rove finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Rove keeps the
...
30 days. You can continue those conversations in Rove.

Conversation import is best effort. Rove keeps the first user prompt and the
...
conversation so Rove does not remove user text. It reads one conversation at
