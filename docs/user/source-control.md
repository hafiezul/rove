...
Rove Code connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

Rove Code works with the platforms your team already uses:
...
- Rove Code can suggest titles and descriptions based on your commits
...
- Command-click (Control-click on Windows and Linux) a pull request number in the sidebar to open it in your browser instead of in Rove Code
...
1. Install the GitHub CLI on the machine running Rove Code:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Rove Code and verify GitHub shows as authenticated
...
machine running Rove Code.

Recommended, a Bitbucket access token:

```bash
export ROVE_BITBUCKET_ACCESS_TOKEN="your-access-token"
```
...
```bash
export ROVE_BITBUCKET_EMAIL="you@example.com"
export ROVE_BITBUCKET_API_TOKEN="your-token"
```

If both are set, the access token wins. Restart Rove Code and verify the connection in **Source
Control settings**.
...
**Git is required** – Rove Code uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Rove Code (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
