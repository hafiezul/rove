# Keeping Rove in Sync

The Rove web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Rove shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                               |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Update server**          | Available for the Rove Linux background service. Select the button and leave Rove open while it prepares, tests, restarts, and reconnects.                               |
| **Update the desktop app** | Open the Rove desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Rove server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Rove does not update connected
...
If the server instead runs as the Rove background service, update the service on the host and
...
See [Running Rove in the Background](./background-service.md) for install, status, and removal
commands.
