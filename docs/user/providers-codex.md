...
This guide is for people who want to use more than one Codex account in Rove. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Rove](./install.md).
...
- both accounts can see the same Rove/Codex sessions
...
In Rove Settings, name it something obvious:
...
In Rove Settings, add another Codex provider:
...
Rove shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Rove treats it
