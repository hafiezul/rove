...
This guide is for people who want to use more than one Codex account in Rove Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Rove Code](./install.md).
...
- both accounts can see the same Rove Code/Codex sessions
...
In Rove Code Settings, name it something obvious:
...
In Rove Code Settings, add another Codex provider:
...
Rove Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Rove Code treats it
