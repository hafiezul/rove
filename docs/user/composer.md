...
also send files to Rove through another app's system share sheet.
...
Rove remembers your provider, model, and model options for new threads. A
...
Transcription runs on your device. Rove deletes the temporary audio after
...
Provider commands must start the message to run. Rove commands such as
...
complete Rove tells you and leaves the chip for you to remove or replace. A chip whose
...
automatically. HTML previews cannot access your Rove session.
