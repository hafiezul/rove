...
Shift keys together. Rove attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Rove is in front captures Rove itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Rove refuses shortcuts
...
terminal contents. If an app is slow to answer, Rove attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Rove SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Rove under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Rove shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Rove shows the exact binding it will
...
**Other Wayland desktops.** Rove uses the desktop's screenshot portal when it can capture the
