...
RUNNER_NEXT="$STATE_DIR/run-rove.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
export class SshInvalidArchiveVersionError extends Schema.TaggedError<SshInvalidArchiveVersionError>()(
  ...
  override get message(): string {
    return `'${this.archiveVersion}' is not an exact rove version and cannot name a runtime directory.`;
  }
}
...
export class SshMissingRunnerError extends Schema.TaggedError<SshMissingRunnerError>()(
  ...
  override get message(): string {
    return "A remote rove runner needs an archive version or a node script path.";
  }
}
