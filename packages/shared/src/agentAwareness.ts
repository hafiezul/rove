...
function resolveThreadAwarenessPhase(
  ...
  // A thread can still reach here with no materialized latestTurn — nothing
  // ever recorded a turn for it. The session status is then the only
  // completion signal: a live session at "ready"/"idle" with nothing pending
  // and nothing running means the agent finished and is waiting for the next
  // prompt — Done.
