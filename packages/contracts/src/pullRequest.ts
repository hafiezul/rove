...
const PROVIDER_REQUIREMENT: ProviderRequirements = {
  ...
  forgejo: {
    ...
    unauthenticated:
      "Authenticate your Forgejo or Gitea server with `fj --host <server-url> auth add-token` on the Rove Code server. If fj is missing or unconfigured for that server, use `tea login add`. A configured fj account must be repaired with fj.",
  },
  gitlab: {
    missing:
  ...
  bitbucket: {
    missing:
      "Bitbucket needs API credentials on the server. Set ROVE_BITBUCKET_EMAIL and ROVE_BITBUCKET_API_TOKEN, or ROVE_BITBUCKET_ACCESS_TOKEN.",
    unauthenticated:
      "Bitbucket rejected the configured credentials. Check ROVE_BITBUCKET_EMAIL and ROVE_BITBUCKET_API_TOKEN.",
  },
};
