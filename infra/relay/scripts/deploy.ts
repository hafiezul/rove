...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    ROVE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    ROVE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    ROVE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    ROVE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    ROVE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    ROVE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("ROVE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("ROVE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "RoveRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Rove Code relay through Alchemy."));
