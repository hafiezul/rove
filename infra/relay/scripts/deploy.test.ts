...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "ROVE_CLERK_PUBLISHABLE_KEY=pk_test_example\nROVE_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "rove-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "rove-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "ROVE_MOBILE_OTLP_TRACES_DATASET=rove-mobile-traces-dev",
        "ROVE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "ROVE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "ROVE_RELAY_CLIENT_OTLP_TRACES_DATASET=rove-relay-client-traces-dev",
        "ROVE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "ROVE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "ROVE_RELAY_URL=https://old.example.test",
          "ROVE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "ROVE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "ROVE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "ROVE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "ROVE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "ROVE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "ROVE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "ROVE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "ROVE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
