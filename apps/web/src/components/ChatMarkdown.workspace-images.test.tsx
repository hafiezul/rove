...
describe("ChatMarkdown workspace images", () => {
  ...
  it("loads every Windows workspace path form through a signed asset URL", () => {
    const imagePath = "C:/Users/shawn/project/.rove/workspace-image.svg";
    const html = render(
      [
        "![relative](.rove/workspace-image.svg)",
        `![absolute](${imagePath})`,
  ...
  it("keeps a tall image placeholder and loaded image at the same proportional bounds", () => {
    const markdown = '<img src=".rove/workspace-image.svg" alt="sized" width="96" height="128">';
  ...
  it.each([
    ...
    const markdown = `<img src=".rove/workspace-image.svg" alt="sized" ${axis}="300">`;
    const loadedStyle = firstInlineStyle(render(markdown));
  ...
  it("keeps images that share a line inline and lets a standalone one reserve a slot", () => {
    const html = render(
      "![remote](https://example.com/badge.svg) ![workspace](.rove/workspace-image.svg)",
    );
    ...
    const centeredHtml = render(
      '<p align="center"><img src=".rove/workspace-image.svg" alt="logo"></p>',
    );
  ...
  it("reserves a slot for an image that is the only content of its link", () => {
    const html = render("[![shot](.rove/workspace-image.svg)](https://example.com)");

    expect(html).toContain("aspect-video");
  });

  it.each([
    ["a link", "Figure: [![shot](.rove/workspace-image.svg)](https://example.com)"],
    ["emphasis", "**![shot](.rove/workspace-image.svg)** caption"],
  ...
  it("sizes the slot from server-reported dimensions so a portrait image never grows", () => {
    ...
    const style = firstInlineStyle(render("![shot](.rove/workspace-image.svg)"));
  ...
  it("lets an authored size override server-reported dimensions", () => {
    ...
    const style = firstInlineStyle(
      render('<img src=".rove/workspace-image.svg" alt="sized" width="96" height="128">'),
    );
  ...
  it("reserves a slot for an image that is alone in a list item", () => {
    expect(render("- ![shot](.rove/workspace-image.svg)")).toContain("aspect-video");
  });
  ...
  it("reserves the same 16:9 frame while the URL, the bytes, and a failure resolve", () => {
    ...
    const markdown = "![shot](.rove/workspace-image.svg)";
