  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Rove Code cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Rove Code cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
// independently of Rove Connect while clients reach the environment out of band.
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "ROVE_RELAY_URL is not configured.",
      });
    }
