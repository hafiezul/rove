...
describe("clientPersistenceStorage", () => {
  ...
  it.each(["not-json", '{"wordWrap":"invalid"}'])(
    ...
    async (value) => {
      ...
      testWindow.localStorage.setItem("rove:client-settings:v1", value);
      ...
      expect(() => readBrowserClientSettings()).toThrow(
        expect.objectContaining({
          ...
          operation: "decode",
          storageKey: "rove:client-settings:v1",
        }),
      );
      expect(testWindow.localStorage.getItem("rove:client-settings:v1")).toBe(value);
    },
  );
  ...
  it("preserves saved settings across a transient read failure", async () => {
    ...
    testWindow.localStorage.setItem("rove:client-settings:v1", JSON.stringify(settings));
  ...
  it("keeps the default diff file state across reloads and defaults it to expanded", async () => {
    ...
    testWindow.localStorage.setItem("rove:client-settings:v1", JSON.stringify({}));
