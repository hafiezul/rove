    ...
    publishMutex.withPermits(1)(
      Effect.gen(function* () {
        ...
        const lease: ClientActivityLease = {
          ...
          ...(input.appState !== undefined ? { appState: input.appState } : undefined),
          ...(input.lowPowerMode !== undefined ? { lowPowerMode: input.lowPowerMode } : undefined),
          ...(input.batteryState !== undefined ? { batteryState: input.batteryState } : undefined),
          ...(input.networkType !== undefined ? { networkType: input.networkType } : undefined),
          scopes: input.scopes,
          updatedAt: now,
          expiresAt,
        };
