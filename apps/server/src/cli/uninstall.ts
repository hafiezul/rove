...
/**
 * What `rove uninstall` would remove for one Rove home. Computed before anything
...
export interface UninstallPlan {
  ...
  /** The `rove` launcher (symlink or `.cmd` shim) that points into this home's runtime tree. */
...
export const uninstallCommand = Command.make("uninstall", {
  ...
  Command.withDescription(
    "Remove rove from this machine: the background service, the launcher, and every downloaded version. Your projects and threads are kept.",
  ),
...
const runUninstall = Effect.fn("cli.uninstall.run")(function* (input: {
  ...
  if (!plan.service && plan.launcher === undefined && plan.runtimeDir === undefined) {
    yield* Console.log(`Nothing to remove: rove is not installed for ${input.baseDir}.`);
    if (!(yield* HostProcessIsExecutable)) {
      yield* Console.log(
        "  This rove runs from a Node script, so it was installed by npm or built from source. Remove it the same way (`npm uninstall -g rove`, or delete the checkout).",
      );
    }
    return;
  }
  ...
  if (!input.assumeYes) {
    ...
    const confirmed = yield* Prompt.run(
      Prompt.confirm({ message: "Remove rove from this machine?", initial: false }),
  ...
  if (plan.runtimeDir !== undefined) {
    ...
    if (platform === "win32") {
      ...
      yield* Console.log(`${runtimeDir} will be removed once rove exits.`);
    } else {
      yield* fs
  ...
  yield* Console.log("");
  yield* Console.log("rove is uninstalled. Thanks for trying Rove Code.");
});
