...
it.layer(
  ...
  it.effect("resolves a provider instance environment before spawning", () =>
    Effect.gen(function* () {
      ...
      expect(ptyAdapter.spawnInputs[0]?.env.ROVE_SECRET).toBeUndefined();
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        ServerSettings.layer.pipe(
          ...
          Layer.provide(
            ServerConfig.layerTest(process.cwd(), { prefix: "rove-terminal-provider-restart-" }),
          ),
        ),
      ),
    ),
