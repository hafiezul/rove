...
const startInput = {
  ...
  destinationPath: "/workspace/rove",
};
...
describe("ProjectCloneTracker", () => {
  it.effect("creates the project first and reports the clone through the stream", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(result.cwd).toBe("/workspace/rove");
      // The project exists before git runs so the draft can open immediately.
      expect(harness.created).toEqual([{ projectId, workspaceRoot: "/workspace/rove" }]);
  ...
  it.effect("keeps a failed clone with git's own explanation and retries it", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(harness.discarded).toEqual(["/workspace/rove"]);
      yield* Effect.yieldNow;
  ...
  it.effect("hands git the credential-bearing URL while snapshots carry the redacted one", () => {
      ...
      Layer.provide(
        Layer.mock(SourceControlRepositoryService.SourceControlRepositoryService)({
          prepareClone: (input) =>
            Effect.succeed({
              destinationPath: input.destinationPath,
              remoteUrl: "https://github.com/octocat/rove.git",
              cloneUrl: "https://user:s3cret@github.com/octocat/rove.git",
              repository: null,
            }),
    ...
    return Effect.gen(function* () {
      ...
      const result = yield* tracker.start(startInput, harness.hooks);
      yield* Effect.yieldNow;
      expect(result.remoteUrl).toBe("https://github.com/octocat/rove.git");
      expect(cloneUrls).toEqual(["https://user:s3cret@github.com/octocat/rove.git"]);
...
describe("parseGitCloneProgressLine", () => {
  it("parses git's transfer counters and ignores other output", () => {
    ...
    expect(parseGitCloneProgressLine("Cloning into 'rove'...")).toBeNull();
