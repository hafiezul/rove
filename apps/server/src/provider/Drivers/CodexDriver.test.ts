...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "rovex-driver-maintenance-",
}).pipe(
...
it.layer(testLayer)("CodexDriver", (it) => {
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "rovex-driver-" });
  ...
  it.effect("stays manual-only when the configured executable does not exist", () =>
    Effect.gen(function* () {
      const instance = yield* CodexDriver.create({
        ...
        config: {
          ...CodexDriver.defaultConfig(),
          binaryPath: NodePath.join(NodeOS.tmpdir(), "rovex-missing", "codex"),
        },
      });
  ...
  for (const fixture of [
    ...
    it.effect.skipIf(windowsHost)(fixture.name, () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "rovex-installer-" });
  ...
  for (const layout of ["direct", "wrapper"] as const) {
    it.effect.skipIf(windowsHost)(`leaves a mise ${layout} installation manual-only`, () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: `rovex-mise-${layout}-` });
        const binaryPath =
          layout === "direct"
  ...
  it.effect.each([
    ...
    (fixture) =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "rovex-mise-shim-" });
