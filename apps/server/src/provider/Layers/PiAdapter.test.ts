...
it.layer(testLayer)("PiAdapter", (it) => {
  ...
  it.effect("keeps extension status snapshots separate from append-only notifications", () =>
    Effect.gen(function* () {
      const fake = new FakePiSession();
      const adapter = yield* makeAdapter(fake);
      ...
      fake.emit({ type: "rove_ui_status", statuses: [{ key: "progress", text: "Step 9" }] });
      fake.emit({ type: "rove_ui_status", statuses: [{ key: "progress", text: "Step 10" }] });
      fake.emit({ type: "rove_ui_status", statuses: [] });
      fake.emit({ type: "rove_ui_text", message: "Pi: Thinking" });
      fake.emit({ type: "rove_ui_notify", level: "info", message: "Done one thing" });

      const events = yield* waitFor(
        eventsRef,
        (entries) =>
          entries.filter((event) => event.type === "runtime.ui.status").length >= 3 &&
          entries.filter((event) => event.type === "runtime.info").length >= 2,
      );
      const statuses = events.filter((event) => event.type === "runtime.ui.status");
      assert.deepEqual(
        statuses.map((event) => event.payload.statuses),
        [[{ key: "progress", text: "Step 9" }], [{ key: "progress", text: "Step 10" }], []],
      );
      assert.isTrue(statuses.every((event) => event.turnId === undefined));
      const info = events.filter((event) => event.type === "runtime.info");
      assert.lengthOf(info, 2);
      assert.strictEqual(info[0]?.itemId, "pi-extension-ui-status");
      assert.strictEqual(info[0]?.turnId, turnId);
      assert.strictEqual(info[0]?.payload.message, "Pi: Thinking");
