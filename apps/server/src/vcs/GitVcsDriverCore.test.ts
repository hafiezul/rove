  ...
  describe("remote operations", () => {
    it.effect("ensureRemote reuses an existing remote across ssh/https transport variants", () =>
      Effect.gen(function* () {
        ...
        yield* git(cwd, ["remote", "add", "origin", "https://github.com/rovedev/rove.git"]);
        ...
        const reusedForSshScheme = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com/rovedev/rove",
        });
        ...
        const reusedForBareSshScheme = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://github.com/rovedev/rove",
        });
        ...
        const reusedForSshPort = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com:22/rovedev/rove",
        });
        ...
        const reusedForSshWithPort = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com:22/rovedev/rove.git",
        });
        ...
        const addedForFork = yield* driver.ensureRemote({
          ...
          url: "git@github.com:octocat/rove.git",
        });
    ...
    it.effect("creates a worktree from the latest fetched remote commit", () =>
      Effect.gen(function* () {
        ...
        yield* driver.createWorktree({
          ...
          newRefName: "rove/fetched-origin",
          baseRefName: resolvedBase.remoteRefName,
        });

        assert.equal(yield* git(worktreePath, ["rev-parse", "HEAD"]), remoteHead);
        assert.equal(
          yield* driver.readConfigValue(worktreePath, "branch.rove/fetched-origin.gh-merge-base"),
          initialBranch,
        );
        assert.equal(
          yield* driver.readConfigValue(worktreePath, "branch.rove/fetched-origin.remote"),
          null,
        );
