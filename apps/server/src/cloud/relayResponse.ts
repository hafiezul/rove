...
export function relayRequestError(cause: unknown) {
  return isRelayResponseError(cause)
    ...
    : new EnvironmentHttpInternalServerError({
        message: `Could not complete the Rove Connect relay request. ${isHttpClientError(cause) ? `The relay request failed (${cause.reason._tag}).` : "The relay returned an unexpected response."} Check this machine's network connection and relay availability, then retry.`,
      });
}
...
export const filterRelayResponse = Effect.fn("cloud.filter_relay_response")(function* (
  ...
  const message = Option.isSome(decoded)
    ? `Rove Connect: ${decoded.value.message}. ${recoveryHint(decoded.value)} Trace ID: ${decoded.value.traceId}.`
    : `Rove Connect relay returned HTTP ${response.status} without a recognized error response. Check relay access and any proxy or firewall restrictions, then restart Rove Code.${requestId}`;
