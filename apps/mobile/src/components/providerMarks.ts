/**
 * Brand marks for the provider drivers the mobile app can display.
 *
 * Fills are declared per theme so a mark stays legible on both light and dark
 * backgrounds. Kept as data (rather than JSX) so the driver -> mark lookup is
 * testable without a React Native renderer.
 */

type MarkFill = { readonly light: string; readonly dark: string };
...
const monochrome = (light: string, dark: string): MarkFill => ({ light, dark });
...
const PROVIDER_MARKS: Readonly<Record<string, ProviderMark>> = {
  ...
  pi: {
    ...
    viewBox: "0 0 800 800",
    shapes: [
      { kind: "rect", width: 800, height: 800, rx: 160, fill: monochrome("#000000", "#ffffff") },
      {
        kind: "path",
        fillRule: "evenodd",
        fill: monochrome("#ffffff", "#000000"),
  ...
  opencode: {
    ...
    shapes: [
      {
        kind: "path",
        fill: monochrome("#CFCECD", "#4B4646"),
        d: "M24 32H8V16H24V32Z",
      },
      {
        kind: "path",
        fill: monochrome("#211E1E", "#F1ECEC"),
...
export function resolveProviderMark(driver: string | null | undefined): ProviderMark | undefined {
  if (driver === null || driver === undefined) return undefined;
  return PROVIDER_MARKS[driver];
}

export function markFill(fill: MarkFill, isDarkMode: boolean): string {
  return isDarkMode ? fill.dark : fill.light;
}
