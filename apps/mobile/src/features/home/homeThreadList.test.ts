...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    expect(scopes[0]?.title).toBe("rove");
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const projects = [
      makeProject({
        ...
        title: "Web",
        workspaceRoot: "/workspaces/rove/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/rove/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
