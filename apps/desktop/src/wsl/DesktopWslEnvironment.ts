...
export class DesktopWslEnvironment extends Context.Service<
  DesktopWslEnvironment,
  {
    ...
    // Proves a staged self-contained runtime can run (`<root>/rove --version`)
...
// Holds the sha256 of the runtime's `rove` executable, written when the install
...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    ...
    // The runtime is a self-contained `rove` executable with Node inside, so the
    // readiness proof is the same one the SSH runner and the CLI installers
    // use: the file is executable and `rove --version` exits 0. That covers the
    // truncated-binary and wrong-arch cases without a separate native probe.
    "runtime_entry_runs() {",
    '  [ -x "$1/rove" ] && "$1/rove" --version >/dev/null 2>&1',
    "}",
    // Hashing the entry is what tells a working cache from one whose `rove` was
    ...
    `  sha256sum "$1/rove" 2>/dev/null | cut -d ' ' -f 1`,
    "}",
    "runtime_is_ready() {",
    '  [ -f "$ready_marker" ] &&',
    ...
    // The release archive has one top-level `rove-<version>-linux-<arch>/`
    // directory; strip it so the executable lands at `$runtime_root/rove`.
    ...
    "  printf 'WSL runtime archive does not contain a working rove executable\\n' >&2",
    "  exit 1",
    "fi",
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.rove-code/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_BINARY_MISSING_EXIT_CODE
    ? "WSL support is missing from this Rove Code build: the packaged Linux node-pty binary was not included. Install a build that includes WSL support."
    : null;
...
export const buildWslRuntimeProbeScript = (linuxAppRoot: string) =>
  [
    ...
    `${shellQuote(`${linuxAppRoot}/rove`)} --version >/dev/null 2>&1`,
  ].join("\n");
...
const probeWslRuntimeImpl = (
  ...
  Effect.gen(function* () {
    ...
    if (probe.exitCode !== 0) {
      ...
      return {
        ok: false,
        reason: `${linuxAppRoot}/rove --version failed (exit ${probe.exitCode})${trimmedTail ? `: ${trimmedTail}` : ""}`,
      } as const;
    }
