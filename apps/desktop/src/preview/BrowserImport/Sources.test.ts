...
const withSourceHome = Effect.fnUntraced(function* () {
  ...
  const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "rove-sources-" });
...
describe("Helium on Linux", () => {
  it.effect.skipIf(!symlinksSupported)("discovers its profiles and checks the user-data lock", () =>
    run(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "rove-helium-linux-" });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, { HOME: home }),
    ...
    run(
      Effect.gen(function* () {
        ...
        const home = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "rove-helium-windows-lock-",
        });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, {
            HOME: home,
...
describe("Firefox Snap profiles", () => {
  ...
  it.effect("keeps matching profile names in native and Snap installs distinct", () =>
    run(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "rove-firefox-snap-" });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, { HOME: home }),
          Effect.provideService(HostProcessPlatform, "linux"),
        );
...
describe("isSourceRunning for Firefox", () => {
  it.effect.skipIf(!symlinksSupported)("finds the lock inside the profile, not at the root", () =>
    run(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "rove-firefox-" });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, { HOME: home }),
          Effect.provideService(HostProcessPlatform, "darwin"),
        );
  ...
  it.effect("reports not-held when no interpreter can run the fcntl probe", () =>
    run(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const directory = yield* fileSystem.makeTempDirectoryScoped({ prefix: "rove-lock-" });
